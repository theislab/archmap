...
import {
  initSubmissionProgress,
  useSubmissionProgress,
} from "shared/context/submissionProgressContext";
...
function UploadFilePage({
  ...
  // validate if it is h5ad or rds
  const validateUploadFile = (file) => {
    if (file.name.split(".").pop() !== "h5ad" && file.name.split(".").pop() !== "rds"  ) {
      setShowFileWarning(true);
      // error object returned in case of rejection
      return {
        code: "Wrong file format",
        message: "File must be in h5ad or rds format",
      };
    }
  ...
  // the function to create a project
  const createProject = useCallback(
    (projectName, atlasId, modelId, file, fileExtension) => {
  ...
  const createDemoProject = (projectName, atlasId, modelId, demoDataset) => {
    ProjectService.createProject(
      ...
      modelId,
      demoDataset.name,
      "h5ad" // demo datasets are always h5ad as of now
    );
    history.push(path); // go back to GeneMapper home
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (datasetIsSelected) {
      ...
      createProject(
        ...
        uploadedFile ? uploadedFile[0].name.split(".").pop() : "h5ad"
      );
    }
  };
    ...
    <Box sx={{ marginTop: "2.5em" }}>
      ...
      >
        ...
        <Box width="50%" mr="3%">
          <Stack direction="column">
            <Typography variant="h5" fontWeight="bold" pb="1em">
              Your Choice
            </Typography>
                ...
                children={
                  <Stack direction="column">
                    ...
                    <Typography
                      gutterBottom
                      variant="caption"
                    >{`Cells in Reference:  ${selectedAtlas.numberOfCells}`}</Typography>
                    <Typography
                      gutterBottom
                      variant="caption"
                    >{`Species: ${selectedAtlas.species}`}</Typography>
                    <Button
                      size="small"
                      variant="outlined"
                      ...
                      children={
                        <Container>
                          <LearnMoreAtlasComponent
                            ...
                            onClick={() =>
                              history.push(
                                `/references/atlases/${selectedAtlas._id}/visualization`
                              )
                            }
                          />
                        </Container>
        ...
        <Box width="50%" ml="3%">
          ...
          <Stack className="flexContainer" direction="column" pb="1em">
            ...
            {showFileWarning && (
              <Alert severity="error" sx={{ marginTop: "1em" }}>
                File must be in h5ad or rds format.
              </Alert>
            )}
          ...
          <Stack maxHeight="50%">
            <Typography variant="h5" fontWeight="bold" mb="0.5em">
      ...
      >
        ...
        <Stack direction="row" spacing={3} alignItems="center">
          ...
          <Tooltip
            title={
              !uploadedFile && !selectedDataset
                ? "You haven't selected or uploaded a dataset!"
                : ""
            }
            placement="top"
          >
